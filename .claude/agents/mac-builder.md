...
1. If `crates/setbuddy-ffi` changed since `apps/mac/Generated/setbuddy.swift` was written
...
3. Separate errors from warnings. For each error: file:line, the message, and the likely
   ...
   API moved: read `apps/mac/Generated/setbuddy.swift` for the new name and say what it is.
