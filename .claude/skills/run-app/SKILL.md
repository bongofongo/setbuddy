---
name: run-app
description: Build and launch Setbuddy.app so a GUI change can be seen working. Use when asked to run, start, relaunch, or screenshot the app.
---

1. `scripts/run-mac-app.sh` (debug). It rebuilds bindings, the Swift package, assembles
   `target/Setbuddy.app`, kills the old instance, and opens the new one. Run it in a tmux
...
3. Logs: `log stream --predicate 'process == "Setbuddy"' --level debug` in another pane.
...
5. To exercise playback without clicking, drive the same core from the CLI while the app
   is open: `cargo run -q -p setbuddy-cli -- play crates/setbuddy-mpv/tests/assets/tiny.webm`.
   They share the state dir and adopt the same mpv.

Stop the app with `pkill -x Setbuddy`. Never `pkill mpv` blindly; the user may have their
own mpv running.
